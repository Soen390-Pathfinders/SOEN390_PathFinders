...
def test_create_campus():
    campus = Campus.objects.create(name="Main Campus", latitude= 45.5017, longitude= -73.5673)
    assert Campus.objects.count() == 1
    assert campus.name == "Main Campus"

@pytest.mark.django_db
def test_create_building():
    campus = Campus.objects.create(name="Science Campus")
    building = Building.objects.create(
        ...
        latitude=45.5017,
        longitude=-73.5673,
        description= "Big bad science building"
    )
...
def test_invalid_polygon_coordinates():
    campus = Campus.objects.create(name="Tech Campus")
    building = Building(
        campus_id=campus,
        name="Tech Building",
        number_of_floors=3,
...
def test_create_floor():
    campus = Campus.objects.create(name="Health Campus")
    building = Building.objects.create(campus=campus, name="Health Sciences", number_of_floors=10)
...
def test_create_room():
    campus = Campus.objects.create(name="Engineering Campus")
    building = Building.objects.create(campus=campus, name="Engineering Block", number_of_floors=7)
    floor = Floor.objects.create(building=building, floor_number=2)
    room = Room.objects.create(floor=floor, room_number="201", room_type="Lecture Hall", capacity=100)
...
def test_create_point_of_interest():
    ...
    building = Building.objects.create(campus=campus, name="Library", number_of_floors=4)
    poi = PointOfInterest.objects.create(building=building, , category="Food", description="Serving Food", opening_hours={})
...
def test_set_opening_hours():
    campus = Campus.objects.create(name="Business Campus")
...
def test_set_invalid_opening_hour():
    campus = Campus.objects.create(name="Art Campus")
